...
from CreateISESpecDocument import createISESpecializationDocument
from CreateCSESpecDocument import createCSESpecializationDocument
from CreateSingularSpecDocument import createSingularSpecDocument
...
def createPlanningGrid(sheet, starter_column, classesPerSemester) -> int:
    ...
    # Print out the name of the courses to find the CSE 114 issue 
    for keys in classesPerSemester.keys():
        listOfClasses = classesPerSemester[keys]
        for values in listOfClasses:
...
def excelFileInputterAlgorithm(sheet, row_value, column_value_start, column_value_end, row_limit, insert_dictionary, class_data, category_dictionary, studentInformation):
    ...
    # Let's take a look at the classes that are introduced
    # Let's check to see if CSE 114 is in here and if not, we are gonna have to go ParseClasses.py to see what is up 
