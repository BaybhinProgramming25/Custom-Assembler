...
from CreateTabularDocument import createDocument
from PlanBackwards import studentInfoBackwards
from StudentInfo import trackStudentInformation, resetGlobalVariablesForStudentInfo 
from CumulativeInfo import trackCumulativeInformation, resetGlobalVariablesForCumulativeInformation
from ParseClasses import *


def main():

        # Before parsing begins, we need to verify that both the correct path is specified (i.e. input\TRANSCRIPTNAME.pdf)
        # and that the transcript also exists inside the input folder
        # The program will run in an infinite-loop until the user specifies 'exit' in the terminal 
        while True:
            directory = ""
            fileFound = 0
            while fileFound != 1:
            ...
            # First check to see if the document already exists and if so, we can simply update the current document
            ...
            if fileOpened == False or fileExists == False:
                    ...
                    shutil.move(fileName, output_folder + fileName) # Move the file to the output folder
